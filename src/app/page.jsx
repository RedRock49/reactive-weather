...
function FavouritesHandle() {
  ...
  let favouriteButtonIcon = favouriteButton.childNodes
    .item(0)
    .childNodes.item(0);
  let favourites = JSON.parse(localStorage.getItem("favourites"));
  if (
    favouriteButtonIcon.getAttribute("src") == "./Button_Icons/Star_empty.svg"
  ) {
    ...
    favourites.splice(
      favourites.indexOf(document.getElementById("selectedCityName").innerHTML),
      1,
    );
...
function OpenFavouriteCity(index) {
  ...
  document
    .getElementById("favourite_button")
    .childNodes.item(0)
    .childNodes.item(0)
    .setAttribute("src", "./Button_Icons/Star_filled.svg");
  fetchData();
}
...
function reloadFavourites() {
  ...
  for (let i = 0; i < favourites.length; i++) {
    //adding new ones
    let mainElementCopy = document
      .getElementById("favouriteCityExample")
      .cloneNode(true);
    mainElementCopy.setAttribute(
      "class",
      "favouriteCity favouriteCityButton Button",
    );
...
export function search() {
  ...
  if (JSON.parse(localStorage.getItem("favourites")).includes(searchQuery)) {
    ...
    document
      ...
      .childNodes.item(0)
      .childNodes.item(0)
      .setAttribute("src", "./Button_Icons/Star_empty.svg");
  }
...
async function fetchData() {
  ...
  document.getElementById("currentConditionsWeatherType").innerHTML =
    data[0].WeatherText;
  document.getElementById("currentConditionsTemp").innerHTML =
    `${sign}${Math.round(data[0].Temperature.Metric.Value)}°`;
  document.getElementById("HForecastNow").childNodes.item(2).innerHTML =
    `${sign}${Math.round(data[0].Temperature.Metric.Value)}°`;

  let conditionsImage = document.getElementById("currentConditionsSVG");
  let HForecastImage = document
    .getElementById("HForecastNow")
    .childNodes.item(1);
  ...
  switch (data[0].WeatherIcon) {
    ...
    case 15:
      conditionsImage.setAttribute(
        "src",
        "./Forecast_Icons/Thunder_anytime.svg",
      ); //thunder(anytime)
      HForecastImage.setAttribute(
        "src",
        "./Forecast_Icons/Thunder_anytime.svg",
      );
      break;
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    forecast.childNodes.item(2).innerHTML =
      Math.round(data[i].Temperature.Value) + "°";
  ...
  if (dailyList.childNodes.length != 1) {
    for (let i = 0; i < 5; i++) {
      dailyList.removeChild(
        document.getElementById(`forecastContainer${i + 1}`),
      );
    }
  }
  for (let i = 0; i < data.DailyForecasts.length; i++) {
    let mainElementCopy = document
      .getElementById("forecastContainer")
      .cloneNode(true);
    mainElementCopy.setAttribute("id", `forecastContainer${i + 1}`);
    mainElementCopy.childNodes
      .item(1)
      .childNodes.item(0)
      .childNodes.item(0).innerHTML =
      "Мин: " + Math.round(data.DailyForecasts[i].Temperature.Maximum.Value);
    mainElementCopy.childNodes
      .item(1)
      .childNodes.item(0)
      .childNodes.item(1).innerHTML =
      "Макс: " + Math.round(data.DailyForecasts[i].Temperature.Minimum.Value);
    mainElementCopy.childNodes
      .item(1)
      .childNodes.item(1)
      .childNodes.item(0).innerHTML = data.DailyForecasts[i].Day.IconPhrase;
    const date = new Date(data.DailyForecasts[i].EpochDate * 1000)
    ...
    switch (data.DailyForecasts[i].Day.Icon) {
      ...
      case 4:
        ...
        mainElementCopy.style.backgroundImage =
      ...
      case 37:
        ...
        mainElementCopy.style.backgroundImage =
          "url('./Forecast_Backgrounds/Fog.jpg')";
        mainElementCopy.style.backgroundPositionY = "center";
        break;
      case 12:
      case 18:
        ...
        mainElementCopy.childNodes
          .item(1)
          .childNodes.item(0)
          .childNodes.item(0).style.color = "#F2F2F2"; //Max
        mainElementCopy.childNodes
          .item(1)
          .childNodes.item(0)
          .childNodes.item(1).style.color = "#F2F2F2"; //Min
        mainElementCopy.childNodes
          .item(1)
          .childNodes.item(1)
          .childNodes.item(0).style.color = "#F2F2F2"; //Condition
        break;
      case 13:
      case 14:
      ...
      case 17:
        ...
        mainElementCopy.childNodes.item(0).childNodes.item(0).style.color =
          "#F2F2F2"; // Date
        mainElementCopy.childNodes
          .item(1)
          .childNodes.item(0)
        ...
        <div id="centralColumn">
          <div id="forecastCurrent">
            <div id="selectedCityGroup">
              ...
              <button
                className="smallButtonButton"
                id="favourite_button"
                type="button"
                onClick={FavouritesHandle}
              >
                <div className="smallButton">
                  <Image
                    className="smallButtonImg"
                    src="./Button_Icons/Star_filled.svg"
            ...
            <Image
              id="currentConditionsSVG"
              src="Forecast_Icons/Cloudy_anytime.svg"
              width={200}
              height={80}
              alt=""
            />
          ...
          <div id="hourlyForecast">
            <div>
              <div className="textContainer">
                <p className="blockText">ПОЧАСОВОЙ ПРОГНОЗ</p>
              </div>
            </div>
            <ul id="listOfHForecasts">
              <HourlyForecast
                id="HForecastNow"
                time="Now"
                weatherCondition="./Forecast_Icons/Cloudy_anytime.svg"
                temp="+16"
              />
            </ul>
          </div>
          <div id="fiveDayForecast">
            <div className="textContainer">
              <p className="blockText">ПРОГНОЗ НА 5 ДНЕЙ</p>
            </div>
            <ul id="dailyContainer">
              <Forecast
                ...
                min="+10"
                max="+20"
                image="./Forecast_Icons/Cloudy_anytime.svg"
                condition="Overcast"
                imageWidth={70}
                imageHeight={70}
              />
